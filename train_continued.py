...
class PPOTrainer:
    def __init__(
        ...
        if resume_from is not None and os.path.exists('swarm_brain.pth'):
            print(f"Loading pre-trained model from epoch {resume_from}...")
            self.network.load_state_dict(torch.load('swarm_brain.pth', map_location=self.device))
    ...
    def train(self, num_epochs: int = 500, start_epoch: int = 1):
        self.global_step = 0
...
def main():
    ...
    trainer.train(num_epochs=150, start_epoch=61)
